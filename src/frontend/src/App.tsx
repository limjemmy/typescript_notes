...
function App() {
  ...
  const handleAddOrEditNote = async () => {
    ...
    if (editId) {
      ...
      const data = await res.json();
      if (data.success) {
        setNotes(notes.map(n => (n.id === editId ? { ...n, title: noteTitle, content: noteContent } : n)));
        setEditId(null);
        setNoteTitle("");
        setNoteContent("");
    ...
    return (
      <div style={{ fontFamily: "Arial, sans-serif", maxWidth: 500, margin: "50px auto", textAlign: "center" }}>
        ...
        <div style={{ marginBottom: 30 }}>
          <h2>Register</h2>
          <input placeholder="Name" style={inputStyle} value={registerForm.name} onChange={(e) => setRegisterForm({ ...registerForm, name: e.target.value })} />
          <input placeholder="Email" style={inputStyle} value={registerForm.email} onChange={(e) => setRegisterForm({ ...registerForm, email: e.target.value })} />
          <input type="password" placeholder="Password" style={inputStyle} value={registerForm.password} onChange={(e) => setRegisterForm({ ...registerForm, password: e.target.value })} />
          <button style={buttonStyle} onClick={handleRegister}>Register</button>
        </div>

        <div style={{ marginBottom: 30 }}>
          <h2>Login</h2>
          <input placeholder="Email" style={inputStyle} value={loginForm.email} onChange={(e) => setLoginForm({ ...loginForm, email: e.target.value })} />
          <input type="password" placeholder="Password" style={inputStyle} value={loginForm.password} onChange={(e) => setLoginForm({ ...loginForm, password: e.target.value })} />
  ...
  return (
    <div style={{ fontFamily: "Arial, sans-serif", maxWidth: 800, margin: "50px auto" }}>
      ...
      <div style={{ marginTop: 20, padding: 20, border: "1px solid #ccc", borderRadius: 10, backgroundColor: "#f9f9f9" }}>
        ...
        <input placeholder="Title" style={inputStyle} value={noteTitle} onChange={(e) => setNoteTitle(e.target.value)} />
        <textarea placeholder="Content" style={{ ...inputStyle, height: 80 }} value={noteContent} onChange={(e) => setNoteContent(e.target.value)} />
